    ...
    namespace detail
    {
        ...
        struct Functionals
        {
            ...
            Functionals(const SymbolMethod &symbolMethod)
            {
                ...
                // Android 15/16 起 LayerMetadata 被移入 android::gui 命名空间，
                ...
                // 选择“不大于当前系统版本”的最大补丁条目，保证 Android 15/16
                // 以及未来版本都能回退到正确的 gui::LayerMetadata 符号。
                auto it = patchesTable.upper_bound(systemVersion);
                if (it != patchesTable.begin()) {
                    --it;
