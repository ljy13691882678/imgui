...
static void tweakAim(float dir) {
    // 快慢基准：速度
    g_cfg.aimSpeed = fclamp(g_cfg.aimSpeed + dir * 0.15f, 0.3f, 3.0f);
    ...
    if (g_cfg.aimMode == 0) {
        g_cfg.pidKp = fclamp(g_cfg.pidKp + dir * 0.01f, 0.05f, 0.5f);
    }
    printf("[tweak] dir=%+.1f speed=%.2f smooth=%.2f\n",
           dir, g_cfg.aimSpeed, g_cfg.smoothX);
}
...
static void drawSimpleAimUi() {
    ...
    if (g_cfg.aimEnabled) {
        ...
        ImGui::Separator();
        ImGui::Text("当前手感：速度 %0.1f  平滑 %.2f  死区 %.3f",
                    g_cfg.aimSpeed, g_cfg.smoothX, g_cfg.deadZone);
