...
struct AimConfig {
    ...
    // ===== ★ 目标数据源（放在结构体【末尾】，保证旧配置文件仍可前缀兼容）=====
    //   PC 数据 = 我们自建 TGCP 网关解密出来的敌人坐标, 经 TCP 送到手机。
    //   0 = 只用本机 YOLO 推理（默认, 与原版行为完全一致, 自瞄手感不变）
    //   1 = 只用 PC 数据（不跑 YOLO: 省电省 CPU, 坐标由 PC 权威解密）
    //   2 = YOLO + PC 混合（两路都画、都能瞄; 与 PC 框重叠的 YOLO 框自动去重）
    //   无论哪一档, 自瞄/扳机/Kalman 跟踪都是同一套代码与参数。
    //   PC 类别约定: 0=敌人 1=AI 2=队友 3=物资 4=死亡盒
    int   pcSourceMode = 0;
